...
public class JsonParser {
  ...
  private SyntaxNode<Value<?, Type>, Token> json(LookaheadIterator<Token> tokens) {
    return element(tokens);
  }

  private SyntaxNode<Value<?, Type>, Token> element(LookaheadIterator<Token> tokens) {
    if (!tokens.hasNext()) {
  ...
  private SyntaxNode<Value<?, Type>, Token> value(LookaheadIterator<Token> tokens) {
  ...
  private SyntaxNode<Value<?, Type>, Token> number(LookaheadIterator<Token> tokens) {
    val token = tokens.next();
    return new JsonSyntaxNode(token, Values.number(token.getLexeme()));
  }

  private SyntaxNode<Value<?, Type>, Token> array(LookaheadIterator<Token> tokens) {
  ...
  private SyntaxNode<Value<?, Type>, Token> object(LookaheadIterator<Token> tokens) {
  ...
  private SyntaxNode<Value<?, Type>, Token> string(LookaheadIterator<Token> tokens) {
  ...
  private LookaheadIterator<Token> tokenize(CharSequence sequence) {
    return LookaheadIterator.wrap(JsonToken.createTokenBuffer().tokenize(sequence).iterator());
  }

  static final class JsonSyntaxNode extends AbstractSyntaxNode<Value<?, Type>, Token> {
