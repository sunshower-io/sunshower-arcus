...
public class CssSelectorParser {
  ...
  private List<SyntaxNode<Symbol, Token>> simpleSelectorSequence(LookaheadIterator<Token> tokens) {
    ...
    if (nextIs(tokens, CssSelectorToken.Identifier, CssSelectorToken.Universal)) {
  ...
  private boolean isComposite(LookaheadIterator<Token> tokens) {
    return nextIs(
        tokens,
        CssSelectorToken.IdentifierSelector,
        CssSelectorToken.Class,
        CssSelectorToken.AttributeGroup,
        CssSelectorToken.PseudoClass,
        CssSelectorToken.PseudoElement,
        CssSelectorToken.FunctionStart,
        CssSelectorToken.Not);
  }
  ...
  private void composite(LookaheadIterator<Token> tokens, List<SyntaxNode<Symbol, Token>> result) {
    ...
    if (type == CssSelectorToken.AttributeGroup) {
      while (tokens.hasNext() && type == CssSelectorToken.AttributeGroup) {
    ...
    } else if (type == CssSelectorToken.PseudoClass || type == CssSelectorToken.PseudoElement) {
      pseudo(tokens, result);
    } else if (type == CssSelectorToken.Not) {
      negation(tokens, result);
    } else {
      val selector = new CssSyntaxNode(symbolForToken(type), tokens.next());
      result.add(selector);
      selector.addChild(expect(tokens, CssSelectorToken.Identifier, CssSelectorToken.Class));
    }
  }
  ...
  private void negation(LookaheadIterator<Token> tokens, List<SyntaxNode<Symbol, Token>> result) {
    val negation = expect(tokens, CssSelectorToken.Not);
    eatWhitespace(tokens);
    ...
    } else if (nextIs(tokens, CssSelectorToken.AttributeGroup)) {
      val results = new ArrayList<SyntaxNode<Symbol, Token>>();
      parseAttributeGroup(tokens, results);
      negation.addChildren(results);
    } else if (nextIs(tokens, CssSelectorToken.PseudoElement, CssSelectorToken.PseudoClass)) {
      val results = new ArrayList<SyntaxNode<Symbol, Token>>();
      pseudo(tokens, results);
      negation.addChildren(results);
    }
    eatWhitespace(tokens);
    expectAndDiscard(tokens, CssSelectorToken.ApplicationEnd);
    result.add(negation);
  }
  ...
  private void pseudo(LookaheadIterator<Token> tokens, List<SyntaxNode<Symbol, Token>> result) {
    val pseudo = expect(tokens, CssSelectorToken.PseudoClass, CssSelectorToken.PseudoElement);
    result.add(pseudo);
    val next = (CssSelectorToken) tokens.peek().getType();
    if (next == CssSelectorToken.FunctionStart) {
      pseudo.addChild(expression(tokens));
    } else {
      val id = expect(tokens, CssSelectorToken.Identifier);
  ...
  private SyntaxNode<Symbol, Token> expression(LookaheadIterator<Token> tokens) {
    eatWhitespace(tokens);
    val function = functionNode(expect(tokens, CssSelectorToken.FunctionStart));
    while (nextIs(
        tokens,
        CssSelectorToken.AdditionOperator,
        CssSelectorToken.Minus,
        CssSelectorToken.Dimension,
        CssSelectorToken.Numeric,
        CssSelectorToken.String,
        CssSelectorToken.Identifier)) {
      function.addChild(
          expect(
              tokens,
              CssSelectorToken.AdditionOperator,
  ...
  private void parseAttributeGroup(
    ...
    val group = expect(tokens, CssSelectorToken.AttributeGroup);
    eatWhitespace(tokens);
    val attribute = expect(tokens, CssSelectorToken.Identifier);
    eatWhitespace(tokens);
    group.addChild(attribute);
    if (!nextIs(
        tokens,
        CssSelectorToken.PrefixOperator,
        CssSelectorToken.SuffixOperator,
        CssSelectorToken.SubstringOperator,
        CssSelectorToken.DashedPrefixOperator,
        CssSelectorToken.StrictEqualityOperator,
        CssSelectorToken.AttributeValueInSetOperator)) {
      expectAndDiscard(tokens, CssSelectorToken.AttributeGroupEnd);
      result.add(group);
      return;
    }
    ...
    val operand = expect(tokens, CssSelectorToken.Identifier, CssSelectorToken.String);
  ...
  private boolean isCombinator(LookaheadIterator<Token> tokens) {
    return nextIs(
        tokens,
        CssSelectorToken.Tilde,
        CssSelectorToken.Whitespace,
        CssSelectorToken.GreaterThan,
        CssSelectorToken.AdditionOperator);
  }
  ...
  public enum ElementSymbol implements Symbol {
    ...
    Addition("+", CssSelectorToken.AdditionOperator),
    ...
    FunctionApplication("<function>()", CssSelectorToken.FunctionStart),
    /** pseudoclass */
