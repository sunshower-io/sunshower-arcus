...
import io.sunshower.arcus.selectors.css.CssSelectorParser.CssSyntaxNode;
...
public interface Expression extends Function<Integer, Integer> {
  ...
  private static Expression parse(LookaheadIterator<SyntaxNode<Symbol, Token>> tokens) {
    ...
    if (is(tokens, CssSelectorToken.AdditionOperator, CssSelectorToken.Minus)) {
  ...
  static void expectNegation(LookaheadIterator<SyntaxNode<Symbol, Token>> tokens) {
    if (is(tokens, CssSelectorToken.Minus)) {
      expect(tokens, CssSelectorToken.Minus);
    } else {
  ...
  static Expression operator(Expression expr, LookaheadIterator<SyntaxNode<Symbol, Token>> tokens) {
    if (is(tokens, CssSelectorToken.AdditionOperator)) {
      expect(tokens, CssSelectorToken.AdditionOperator);
  ...
  static Expression readFunctionalPrelude(LookaheadIterator<SyntaxNode<Symbol, Token>> tokens) {
    if (is(tokens, CssSelectorToken.Numeric)) {
      val n = number(tokens);
      if (is(tokens, CssSelectorToken.Identifier)) {
  ...
  static Expression variable(LookaheadIterator<SyntaxNode<Symbol, Token>> tokens) {
    return new Variable(expect(tokens, CssSelectorToken.Identifier).getSource().getLexeme());
  }

  static Expression number(LookaheadIterator<SyntaxNode<Symbol, Token>> tokens) {
    return new Constant(parseInt(expect(tokens, CssSelectorToken.Numeric).getSource().getLexeme()));
  }
